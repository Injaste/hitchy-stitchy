...
--   (Changelog gap: the 20260615–20260627 migrations are not itemised here —
--    incl. the invitation per-day/segment redesign and the plan-tiers layer.
--    Some RPC bodies were refolded into this file; the plan-tiers helper layer
--    (effective_plan_key / plan_within_limits / plan_has_feature / plans, etc.)
--    was NOT — see supabase/migrations/ for the authoritative current state.)
...
--   20260806000003_propagate_group_code_rank            — get_bootstrap_context /
--       get_members now ship `code` + `rank` on the access-group payload, not
--       just id/name/permissions. Retires two client fragilities: the invite
--       default was matching the display LABEL ("Helper") instead of the stable
--       code, and lib/access/policy.ts's rank was inferred from
--       permissions.members === 'full' rather than the real rank column (that
--       inference only stayed correct by coincidence once 'members' got locked
...
-- get_bootstrap_context  [last updated: 20260806000003_propagate_group_code_rank]
-- Body confirmed against the LIVE DB (schema.sql had drifted badly — missing
-- the entire plan/catalog block added by the tiered-plans work; replaced
...
RETURNS event_invitations LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  v_caller event_members; v_inv event_invitations; v_mode event_rsvp_mode; v_code text;
  v_plan_cap integer; v_max_guests integer;
BEGIN
  ...
  -- Page capacity can't exceed the plan cap; an unset cap DEFAULTS to it.
  ...
  UPDATE event_invitations SET
    ...
    rsvp_deadline = p_rsvp_deadline, max_guests = v_max_guests,
