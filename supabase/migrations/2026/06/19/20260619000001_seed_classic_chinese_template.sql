-- Migration: seed the `classic-chinese` invitation template catalog row.
-- =============================================================================
-- Festive traditional Chinese design (cream + vermilion + gold, 双喜 double-
-- happiness motif). template_key matches the code registry key. Idempotent via
-- NOT EXISTS on template_key.
-- =============================================================================
...
SELECT
  'Classic Chinese',
  'classic-chinese',
  'Festive and traditional — cream, vermilion red and gold with the 囍 double-happiness motif. The quintessential Chinese wedding look.',
  '{
    "groom_name": "Jun Wei",
    "bride_name": "Mei Ling",
    "font_couple": null,
    ...
    "quote": "百年好合",
    "quote_source": "A hundred years of harmony",
    "section_title": "A Celebration of Love",
    "invitation_body": "Together with our families, we joyfully invite you to celebrate our marriage — two families joined as one, in love and lasting happiness.",
    ...
    "page_description": "Together with our families, we joyfully invite you to celebrate our marriage.",
    "og_image": null
  }'::jsonb,
  true
WHERE NOT EXISTS (SELECT 1 FROM public.event_templates WHERE template_key = 'classic-chinese');

-- Rollback: DELETE FROM public.event_templates WHERE template_key = 'classic-chinese';
