-- Consolidate to ONE rsvp enum: drop `source`, rebuild event_rsvp_mode w/o 'both'.
-- =============================================================================
-- event_rsvp_source duplicated event_rsvp_mode (both public/private). With `both`
-- gone, a page's mode fully determines its guests — a private page's rows ARE the
-- reserved list (matched by phone), a public page's are open RSVPs. So `source` is
-- redundant: drop the column + its enum. And finish removing `both` by recreating
-- event_rsvp_mode as ('public','private') — Postgres can't drop an enum value, so
-- rename → create new → retype the column → recreate the one dependent function
-- (update_invitation) → drop the old type.
--
-- Wrapped in a transaction: all-or-nothing. Touches four live RPCs + the schema.
-- Run as one script.
-- =============================================================================
BEGIN;

-- ── Part A: retire `source` ──────────────────────────────────────────────────
-- A1) rewrite the three source-writing RPCs without it (bodies = the live
--     20260618000008 shapes, source removed; private match is now phone-only since
--     every row on a private page is a pre-loaded reserved guest).
...
BEGIN
  ...
  -- PRIVATE: reserved-only. Every row on a private page is a pre-loaded guest, so a
  -- phone match (any status) is the reserved seat; the shared code is the gate.
...
-- A2) drop the now-unused column + enum.
ALTER TABLE public.event_rsvps DROP COLUMN source;
DROP TYPE public.event_rsvp_source;

-- ── Part B: rebuild event_rsvp_mode as ('public','private') ──────────────────
-- Safety: collapse any stray 'both' page to public before retyping.
UPDATE public.event_invitations SET rsvp_mode = 'public' WHERE rsvp_mode::text = 'both';

-- update_invitation is the only function with the type in its signature — drop it
-- so the type can be replaced, then recreate it against the new type.
...
ALTER TYPE public.event_rsvp_mode RENAME TO event_rsvp_mode_old;
CREATE TYPE public.event_rsvp_mode AS ENUM ('public', 'private');
ALTER TABLE public.event_invitations ALTER COLUMN rsvp_mode DROP DEFAULT;
ALTER TABLE public.event_invitations
  ALTER COLUMN rsvp_mode TYPE public.event_rsvp_mode
  USING rsvp_mode::text::public.event_rsvp_mode;
ALTER TABLE public.event_invitations ALTER COLUMN rsvp_mode SET DEFAULT 'public';
DROP TYPE public.event_rsvp_mode_old;
...
-- Rollback: re-run 20260618000008 bodies, then ADD COLUMN source + recreate the
-- event_rsvp_source enum and the 'both' enum value (manual; this is a one-way trim).
