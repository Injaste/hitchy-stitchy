-- Migration: Subscription plans (109) — plan gates on RSVP / guest writes
-- =============================================================================
-- These RPCs are on the public-page danger list, so they're added alongside the
-- live ones (expand) and swapped at cutover (contract), NOT replaced in place.
--
-- The guest cap is TWO caps (see plan_allows, migration 104): ACTIVE
-- (non-cancelled) ≤ max_guests, and TOTAL (incl cancelled) ≤ max_guests +
-- cancelled_grace_pct. So cancelled is free up to the grace, then counts — no
-- fake-cancelled loophole. Every path that GROWS the count checks it; cancel /
-- shrink / delete are free (delete is the release valve).
--
--   • create_guest            the renamed + gated successor to
--                             create_guest_on_pages (which stays until the FE
--                             repoints + we drop it). create_guests (2-arg) is
--                             dead. Adds assert_event_writable + per-row
--                             assert_plan('guests') (the two-cap check).
--   • submit_rsvp_v2          public RSVP + the plan cap (active+total), on top of
--                             the existing per-page seating check. Reuses the
--                             guest-friendly "reached maximum capacity" message —
--                             never "upgrade" on the public path.
--   • update_invitation_v2    clamps invitation.max_guests to max_guests (seating
--                             = active) and DEFAULTS an unset cap to it, so the
--                             per-page check always carries the limit. + the lock.
--   • update_guest_v2         the lock + a guest check that fires ONLY when the
--                             edit grows active count (bigger party / un-cancel).
--
-- Cutover (later): point the new FE at these, test E2E, then drop the originals
-- (+ create_guest_on_pages) and rename the _v2 trio to their base names.
-- Bodies are the live 20260618000009 shapes; only the marked lines are new.
-- =============================================================================

-- ── create_guest (renamed + gated create_guest_on_pages) ─────────────────────
CREATE OR REPLACE FUNCTION public.create_guest(p_event_id uuid, p_invitation_ids uuid[], p_guest jsonb)
...
BEGIN
  ...
  PERFORM assert_event_writable(p_event_id);   -- NEW: paid/active + not over-limit
  ...
  FOREACH v_page IN ARRAY p_invitation_ids LOOP
    ...
    PERFORM assert_plan(p_event_id, 'guests', v_count);   -- NEW: event guest cap, per added row

    -- A private page's guests are matched by phone when they claim, so phone is required.
    IF v_inv.rsvp_mode = 'private' AND v_phone IS NULL THEN
    ...
    VALUES (
      p_event_id, v_page, v_name, v_phone, v_count, p_guest->>'message', v_status,
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.create_guest(uuid, uuid[], jsonb) TO authenticated;

-- ── submit_rsvp_v2 (public RSVP + plan cap: active + total-with-grace) ────────
...
BEGIN
  ...
  -- NEW: plan cap (active + total-with-grace; guards pages with no max_guests set).
  -- Guest-friendly message — a public guest can't "upgrade".
  IF NOT plan_allows(v_inv.event_id, 'guests', v_guest_count) THEN
...
-- ── update_invitation_v2 (clamp page cap to plan + lock) ──────────────────────
CREATE OR REPLACE FUNCTION public.update_invitation_v2(
  p_event_id            uuid,
  p_id                  uuid,
  p_template_key        text,
...
DECLARE
  ...
  v_code       text;
  v_plan_cap   integer;   -- NEW
  v_max_guests integer;   -- NEW
BEGIN
  ...
  -- NEW: page capacity can't exceed the plan cap; an unset cap DEFAULTS to it, so
  -- the per-page enforcement in submit_rsvp always carries the plan limit.
...
DECLARE
  ...
  v_count      integer;
  v_moving     boolean;
  v_old_active integer;   -- NEW
  v_new_active integer;   -- NEW
BEGIN
  ...
  -- NEW: guest cap — only when this edit GROWS the event's active guest count
  -- (a bigger party, or un-cancelling). Cancelling / shrinking is always free.
  v_old_active := CASE WHEN v_rsvp.status <> 'cancelled' THEN v_rsvp.guest_count ELSE 0 END;
  v_new_active := CASE WHEN COALESCE(p_status, v_rsvp.status) <> 'cancelled'
                       THEN COALESCE(p_guest_count, v_rsvp.guest_count) ELSE 0 END;
  IF v_new_active > v_old_active THEN
    PERFORM assert_plan(p_event_id, 'guests', v_new_active - v_old_active);
  END IF;
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.update_guest_v2(uuid, uuid, text, text, integer, text, event_rsvp_status, uuid) TO authenticated;

-- Rollback:
--   DROP FUNCTION public.create_guest(uuid, uuid[], jsonb);
--   DROP FUNCTION public.submit_rsvp_v2(uuid, jsonb, text);
--   DROP FUNCTION public.update_invitation_v2(uuid, uuid, text, jsonb, event_rsvp_mode, timestamptz, integer, integer, integer, text, jsonb, text, boolean);
--   DROP FUNCTION public.update_guest_v2(uuid, uuid, text, text, integer, text, event_rsvp_status, uuid);
