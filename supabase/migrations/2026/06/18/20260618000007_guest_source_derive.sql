-- Derive a guest's source from the page's RSVP mode (no more manual source picks).
-- =============================================================================
-- The page mode defines the guest type: a private page's guests are Reserved, a
-- public page's are Public, and a "both" page takes the admin's choice. So instead
-- of trusting a per-guest `source` for every page, the server DERIVES it per page;
-- the admin form only sends a choice that matters for both-mode pages. A Reserved
-- guest must have a phone (their identity when they claim a seat).
--
-- CREATE OR REPLACE only — same signatures, grants preserved. submit_rsvp and all
-- read paths untouched. Bodies are the canonical 20260618000006 shapes with the
-- derivation + reserved-phone rule added.
-- =============================================================================

-- ── create_guest_on_pages — derive source per page from the page's mode ───────
...
DECLARE
  ...
  v_name   text;
  v_phone  text;
  v_choice event_rsvp_source;  -- admin's pick; only used for a "both" page
  v_source event_rsvp_source;  -- the type actually stored, derived per page
...
BEGIN
  ...
  v_choice := COALESCE(NULLIF(p_guest->>'source', '')::event_rsvp_source, 'private');
  ...
  FOREACH v_page IN ARRAY p_invitation_ids LOOP
    ...
    -- The page's mode defines the type: single-mode pages dictate it; a both page
    -- uses the admin's choice.
    v_source := CASE WHEN v_inv.rsvp_mode = 'both'
                     THEN v_choice
                     ELSE v_inv.rsvp_mode::text::event_rsvp_source END;
...
-- ── update_guest — on a move, the destination page redefines the type ─────────
...
BEGIN
  ...
  -- On a move, the destination's mode redefines the type: a single-mode page
  -- dictates it; a both page keeps the guest's current source.
  IF v_moving AND v_inv.rsvp_mode <> 'both' THEN
    v_source := v_inv.rsvp_mode::text::event_rsvp_source;
  ELSE
...
-- Rollback: re-run 20260618000006 to restore the pre-derivation bodies.
