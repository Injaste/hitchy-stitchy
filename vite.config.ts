...
export default defineConfig(({ mode }) => {
  // Fail fast on a missing canonical base URL — it's required (the shareable
  // wedding/invite links are built from it), so surface it in the terminal now
  // rather than rendering broken links at runtime.
  const env = loadEnv(mode, process.cwd())
  if (!env.VITE_BASE_URL) {
    throw new Error(
      "VITE_BASE_URL is required — set it in .env.local (e.g. https://hitchystitchy.com)",
    )
  }
