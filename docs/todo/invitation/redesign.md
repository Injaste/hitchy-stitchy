...
> runs on the new per-(day, segment) model. **Step 4 (private mode) has shipped**
> (now a single public/private model — no `both`/`source`). What's left:
> **old-model cleanup** (below) + the deferred items.
>
