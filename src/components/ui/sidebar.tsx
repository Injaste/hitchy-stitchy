...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* No in-flow gap: this app reserves the sidebar's space via SidebarInset's
          animated `marginLeft` (see SidebarInset) rather than the stock gap spacer.
          A non-zero gap here would double-count that offset now that the sidebar
          renders in-tree (it used to be portaled out, which hid the gap). The
          fixed container below still floats over the reserved margin. */}
...
// ── Inset ────────────────────────────────────────────────────────────────────
// marginLeft driven by framer-motion + sidebar context (no peer selectors). The
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-xl px-2 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className,
      )}
      {...props}
    />
...
function SidebarMenuItem({
  ...
  const ctx = React.useContext(SidebarHoverContext);

  return (
    <li
      ref={ctx && id ? ctx.setRef(id) : undefined}
      onMouseEnter={ctx && id ? () => ctx.onMouseEnter(id) : undefined}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-xl p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-all group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 data-active:text-sidebar-primary-foreground active:bg-sidebar-foreground/10 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground group-data-[collapsible=icon]:rounded-full",
  {
    variants: {
      variant: {
