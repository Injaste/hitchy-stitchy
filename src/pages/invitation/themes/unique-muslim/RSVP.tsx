...
const RSVP = ({ eventConfig }: { eventConfig: PublicEventConfig }) => {
  ...
  const handleSubmit = async (value: RSVPFormData) => {
    if (isEditing) { await update.mutate(value) } else { await submit.mutate(value) }
    setIsEditing(false)
    fireConfetti()
  }
  ...
  if (isLoading) {
    return <div className="py-32 text-center text-primary font-bold italic">Checking RSVP status…</div>
  }

  return (
    <section id="rsvp" className="pt-20 sm:pt-32 pb-10 sm:pb-12 px-4 sm:px-6 relative bg-white/10 backdrop-blur-sm z-10">
      <div className="max-w-sm sm:max-w-md mx-auto">
        ...
        >
          <motion.div variants={fadeIn(0)} className="text-center mb-8 sm:mb-10">
            <motion.div variants={fadeIn(0.05)}>
              <Heart className="text-primary mx-auto mb-4 sm:mb-6 fill-primary/10" size={40} />
            </motion.div>
            <motion.h2 variants={fadeUp(0.15, 16, 0.7)} className="text-3xl sm:text-4xl font-bold text-primary mb-2 sm:mb-3 italic">
              RSVP
            </motion.h2>
            <motion.p variants={fadeUp(0.25, 12, 0.7)} className="text-muted-foreground italic text-sm sm:text-base">
          ...
          >
            <div ref={bodyRef}>
              {eventConfig.rsvp_mode === "private" ? (
                <motion.div key="pool-closed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-8">
                ...
                <motion.div key="deadline-closed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-8">
                  <p className="text-foreground/70 italic text-sm sm:text-base leading-relaxed">
                  ...
                  {existingRSVP && !isEditing ? (
                    ...
                    >
                      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, damping: 20 }}>
                        <CheckCircle2 className="text-green-400 mx-auto mb-4 sm:mb-6" size={64} />
                      </motion.div>
                      <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-foreground">Alhamdulillah!</h3>
                      ...
                      <div className="flex gap-3 justify-center">
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                          <Button variant="outline" size="sm" onClick={() => setIsEditing(true)} className="rounded-xl border-primary/30 hover:border-primary/60 gap-2 font-bold text-xs tracking-wide uppercase shrink-0">
                            <Edit2 size={14} className="text-primary" /> Edit
                          </Button>
                        </motion.div>
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                          <Button variant="outline" size="sm" disabled={remove.isPending} onClick={() => setShowDeleteDialog(true)} className="rounded-xl border-primary/30 hover:border-destructive/60 hover:text-destructive gap-2 font-bold text-xs tracking-wide uppercase shrink-0">
                    ...
                    >
                      <RSVPForm
                        ...
                        defaultValues={isEditing && existingRSVP ? {
        ...
        <Footer couple_names={eventConfig.couple_names} />
      </div>

      <RSVPDelete open={showDeleteDialog} onConfirm={handleDeleteConfirm} onCancel={() => setShowDeleteDialog(false)} />
    </section>
  )
}
