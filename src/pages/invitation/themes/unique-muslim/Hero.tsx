...
const greetingV = fadeUp(T.greeting, 16, 0.9)
const dividerV = divider(T.divider)
const name1V = fadeUp(T.name1, 40, 1.1)
const ampV = fadeIn(T.amp, 0.7)
const name2V = fadeUp(T.name2, 40, 1.1)
const countdownV = fadeUp(T.countdown, 20, 0.8)
const verseV = fadeUp(T.verse, 16, 0.8)
const ctaV = fadeUp(T.cta, 12, 0.7)
...
const Hero = ({ eventConfig }: { eventConfig: PublicEventConfig }) => {
  ...
  return (
    <section className="relative min-h-svh flex flex-col items-center justify-center text-center py-20 sm:py-32 px-4 sm:px-6 overflow-hidden bg-white/10 backdrop-blur-sm">
      <motion.div initial="hidden" animate="show" className="z-10 w-full max-w-2xl mx-auto">
        <motion.p variants={greetingV} className="text-foreground/80 text-lg sm:text-2xl tracking-wider mb-14 sm:mb-20">
          {greetingText}
        </motion.p>

        <motion.div variants={dividerV} style={{ originX: "50%" }} className="flex items-center justify-center gap-5 mb-5">
        ...
        <div className="mb-12 sm:mb-16">
          ...
          <motion.div variants={ampV} className="flex items-center justify-center gap-5 my-2 sm:my-4">
            <div className="h-px flex-1 max-w-16 bg-primary/25" />
            <span className="text-xl sm:text-2xl font-light not-italic tracking-normal text-foreground/40">&amp;</span>
            <div className="h-px flex-1 max-w-16 bg-primary/25" />
          </motion.div>
        ...
        {weddingDate && (
          <motion.div variants={countdownV} className="mb-14 sm:mb-20">
            <CountdownTimer targetDate={weddingDate} />
          </motion.div>
        )}

        <motion.div variants={verseV} className="mb-14 sm:mb-20">
          <div className="border-t border-primary/20 pt-6 sm:pt-8">
            <p className="text-foreground leading-relaxed text-base sm:text-lg">"{quoteText}"</p>
