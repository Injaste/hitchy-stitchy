...
import type { InkChinesePageConfig } from "./types"
import { rsvpClassNames, rsvpLabels, rsvpDeleteClassNames, rsvpDeleteLabels } from "./form"
import { inkChineseAnchors } from "./anchors"
...
const CONFETTI_COLORS = ["#2a2723", "#b0392c", "#6f6a5e", "#f4f0e5", "#b0392c"]

// ─── Ink motifs ──────────────────────────────────────────────────────────────

const Seal = ({ size = 44, char = "喜" }: { size?: number; char?: string }) => (
  <span
    className="ik-hanzi inline-grid place-items-center text-(--ik-on-ink) bg-(--ik-seal) leading-none rounded-[3px]"
    style={{ width: size, height: size, fontSize: size * 0.56, transform: "rotate(-3deg)" }}
    aria-hidden
  >
    {char}
  </span>
...
const BrushStroke = ({ className = "" }: { className?: string }) => (
  <svg viewBox="0 0 200 16" className={className} aria-hidden preserveAspectRatio="none">
    <path
      d="M4 9 C 50 3, 90 13, 130 7 S 192 9, 196 8 L 196 11 C 150 16, 90 14, 50 12 C 28 11, 12 12, 4 12 Z"
      fill="var(--ik-ink)"
      opacity="0.82"
    />
  </svg>
    ...
    <div className="flex items-end justify-center gap-6">
      {UNITS.map((u) => (
        <div key={u.key} className="flex flex-col items-center w-16">
          <span className="ik-countdown-number text-3xl text-(--ik-fg) tabular-nums leading-none">{String(t[u.key]).padStart(2, "0")}</span>
          <span className="mt-1.5 text-3xs uppercase tracking-[0.2em] text-(--ik-muted-fg)">{u.label}</span>
        </div>
      ))}
    </div>
  )
...
const Preloader = ({ loaderReady, onExitComplete }: { loaderReady: boolean; onExitComplete: () => void }) => {
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div className="fixed inset-0 z-100 flex items-center justify-center bg-(--ik-bg)" initial={{ opacity: 1 }} animate={{ opacity: leaving ? 0 : 1 }} transition={{ duration: 0.8, ease: "easeInOut" }} onAnimationComplete={() => { if (leaving) setVisible(false) }}>
          <motion.div initial={{ opacity: 0, scale: 0.6, rotate: -8 }} animate={{ opacity: 1, scale: 1, rotate: -3 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}>
            <Seal size={84} />
          </motion.div>
        </motion.div>
      )}
...
const InkChinese = ({ eventConfig, pageConfig, loaderReady }: ThemeProps) => {
  const config = (pageConfig ?? {}) as Partial<InkChinesePageConfig>
  const fonts = useMemo(() => ({
  ...
  const renderClosed = (key: string, message: string) => (
    <motion.p key={key} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center text-(--ik-muted-fg) italic leading-relaxed py-8">{message}</motion.p>
  )
  const renderSuccess = () => (
    <div key="success" className="text-center">
      ...
      <motion.div key="success-content" initial={rsvp.submitted ? { opacity: 0, y: 12 } : false} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
        <h3 className="text-2xl font-medium my-3 text-(--ik-ink) italic">{config.rsvp_success_heading}</h3>
        <motion.p initial={rsvp.submitted ? { opacity: 0 } : false} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.6, ease: "easeOut" }} className="text-(--ik-muted-fg) leading-relaxed italic mb-6">{eventConfig.confirmation_message}</motion.p>
        <motion.div initial={rsvp.submitted ? { opacity: 0 } : false} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.5, ease: "easeOut" }} className="flex gap-3 justify-center">
          <Button variant="outline" size="sm" onClick={() => rsvp.setIsEditing(true)} className="rounded-none bg-transparent text-(--ik-fg) border-(--ik-border) hover:bg-(--ik-bg-2) gap-2 font-semibold tracking-widest uppercase text-xs shrink-0"><Edit2 size={14} className="text-(--ik-ink)" /> Edit</Button>
          <Button variant="outline" size="sm" disabled={rsvp.removePending} onClick={() => rsvp.setShowDeleteDialog(true)} className="rounded-none bg-transparent text-(--ik-fg) border-(--ik-border) hover:border-(--ik-destructive)/50 hover:text-(--ik-destructive) gap-2 font-semibold tracking-widest uppercase text-xs shrink-0"><Trash2 size={14} className="text-(--ik-ink)" />{rsvp.removePending ? "Removing…" : "Delete"}</Button>
        </motion.div>
      </motion.div>
    </div>
  )
  ...
  const anchorItems = inkChineseAnchors.items.filter((item) => !item.when || item.when(config))

  return (
    <div className="ik-root relative min-h-svh bg-(--ik-bg) text-(--ik-fg)" style={rootStyle}>
      <Preloader loaderReady={!!loaderReady} onExitComplete={() => setReady(true)} />

      {/* ── Hero ── seal + brush, ink names */}
      <section id="hero" className="relative min-h-svh flex flex-col items-center justify-center text-center px-6 py-20">
        <motion.div initial="hidden" animate={ready ? "show" : "hidden"} className="w-full max-w-lg mx-auto">
          <motion.div variants={heroMake(0)} className="flex justify-center mb-8"><Seal size={52} /></motion.div>
          <motion.p variants={heroMake(0.2)} className="text-(--ik-muted-fg) text-base italic mb-8 whitespace-pre-line ik-hanzi">{config.greeting}</motion.p>
          <motion.div variants={heroMake(0.4)} className="mx-auto mb-6 w-36 text-(--ik-ink)"><BrushStroke className="block h-3 w-full" /></motion.div>
          <motion.p variants={heroMake(0.55)} className="text-(--ik-muted-fg) text-xs tracking-[0.4em] uppercase mb-5">{config.hero_divider_label}</motion.p>
          <motion.h1 variants={heroMake(0.75, 22, 1)} className="ik-couple-names text-5xl sm:text-6xl text-(--ik-fg) leading-tight">{config.groom_name}</motion.h1>
          <motion.p variants={heroMake(1.0)} className="ik-couple-names text-2xl text-(--ik-seal) my-2">&amp;</motion.p>
          <motion.h1 variants={heroMake(1.15, 22, 1)} className="ik-couple-names text-5xl sm:text-6xl text-(--ik-fg) leading-tight">{config.bride_name}</motion.h1>
          ...
          <motion.div variants={heroMake(1.8)} className="mt-10">
            <div className="mx-auto mb-6 w-36 text-(--ik-ink)"><BrushStroke className="block h-3 w-full" /></div>
            <p className="text-(--ik-fg)/80 leading-relaxed italic whitespace-pre-line max-w-md mx-auto ik-hanzi">{config.quote}</p>
            <span className="block mt-3 text-(--ik-muted-fg) text-xs tracking-[0.25em] uppercase">{config.quote_source}</span>
          </motion.div>
        </motion.div>
      </section>

      {/* ── Invitation + Details ── card-less, ruled */}
      <section id="details" className="py-20 px-6">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} className="max-w-md mx-auto text-center">
          <motion.div variants={fadeIn(0)} className="flex justify-center mb-6"><Seal size={34} /></motion.div>
          <motion.h2 variants={fadeUp(0.1, 14, 0.7)} className="text-3xl font-medium italic text-(--ik-ink) mb-5">{config.section_title ?? "A Celebration of Love"}</motion.h2>
          {config.invitation_body && <motion.p variants={fadeUp(0.2, 14, 0.8)} className="text-(--ik-muted-fg) leading-relaxed mb-12">{config.invitation_body}</motion.p>}
          {(config.blessings_name || config.blessings_label) && (
            <motion.div variants={fadeIn(0.25)} className="mb-12">
              <p className="text-(--ik-muted-fg) text-sm mb-2">{config.blessings_prefix}</p>
              {config.blessings_name && <h3 className="text-2xl font-medium text-(--ik-ink) whitespace-pre-line italic">{config.blessings_name}</h3>}
              {config.blessings_label && <p className="text-(--ik-muted-fg) text-sm mt-1">{config.blessings_label}</p>}
            </motion.div>
          )}
          {detailsList.length > 0 && (
            <motion.div variants={fadeIn(0.3)} className="flex flex-col">
              {detailsList.map((item, idx) => (
                <motion.div key={idx} variants={fadeUp(idx * 0.08, 10, 0.6)} className="py-4 border-t border-(--ik-border) last:border-b">
                  <p className="text-2xs uppercase tracking-[0.3em] text-(--ik-muted-fg) mb-1">{item.title}</p>
                  <p className="text-lg text-(--ik-fg)">{item.detail}</p>
                </motion.div>
              ))}
            </motion.div>
          )}
        ...
        <section id="itinerary" className="py-20 px-6">
          <div className="max-w-md mx-auto text-center">
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }} className="mb-10">
              <motion.div variants={fadeIn(0)} className="flex justify-center mb-4"><Seal size={30} /></motion.div>
              <motion.h2 variants={fadeUp(0.1, 14, 0.7)} className="text-3xl font-medium italic text-(--ik-ink)">{config.itinerary_title ?? "Programme"}</motion.h2>
            </motion.div>
            <div className="flex flex-col">
              {itinerarySections.map((section, si) => (
                <motion.div key={si} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-40px" }} variants={fadeUp(0, 14, 0.6)}>
                  {si > 0 && <div className="mx-auto my-7 w-24 text-(--ik-ink)"><BrushStroke className="block h-3 w-full" /></div>}
                  <p className="ik-couple-names text-2xl text-(--ik-ink) mb-3">{section.title}</p>
                  <div className="flex flex-col gap-1.5 text-(--ik-muted-fg)">
                    {section.items.map((item, ii) => (
                      <div key={ii} className="flex items-baseline justify-center gap-3 text-sm">
                        <span className="tabular-nums text-(--ik-seal)">{item.time}</span>
            ...
            {config.footnote && <motion.p initial="hidden" whileInView="show" viewport={{ once: true, margin: "-40px" }} variants={fadeIn(0.1)} className="mt-10 text-(--ik-muted-fg) italic text-sm">{config.footnote}</motion.p>}
          </div>
        </section>
      )}

      {/* ── RSVP ── ruled, card-less */}
      <section ref={rsvp.sectionRef} id="rsvp" className="py-20 px-6">
        <div className="max-w-md mx-auto">
          <motion.div layout transition={{ layout: { duration: 0.45, ease: [0.16, 1, 0.3, 1] } }} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }} variants={fadeIn(0, 0.8)} className="border-y-2 border-(--ik-ink)/20 py-10">
            <div className="text-center mb-8">
              <motion.div variants={fadeIn(0)} className="flex justify-center mb-4"><Seal size={34} /></motion.div>
              <motion.h2 variants={fadeUp(0.1, 12, 0.6)} className="text-3xl font-medium text-(--ik-ink) italic">RSVP</motion.h2>
              <motion.p variants={fadeUp(0.2, 10, 0.6)} className="text-(--ik-muted-fg) italic mt-2 text-sm">{config.rsvp_subtitle}</motion.p>
            </div>
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-40px" }} className="mt-14 text-center">
            <motion.div variants={fadeIn(0)} className="flex justify-center mb-5"><Seal size={30} /></motion.div>
            <motion.p variants={fadeUp(0.1, 10, 0.6)} className="text-2xs uppercase tracking-[0.3em] text-(--ik-muted-fg) mb-3">{config.footer_tagline}</motion.p>
            <motion.h2 variants={fadeUp(0.2, 12, 0.7)} className="ik-couple-names text-3xl text-(--ik-fg)">{config.groom_name} &amp; {config.bride_name}</motion.h2>
          </motion.div>
        </div>
      ...
      <AnchorDock ready={ready} eventConfig={eventConfig} scrollItems={anchorItems}
        classNames={inkChineseAnchors.classNames} drawerClassNames={inkChineseAnchors.drawer} labels={inkChineseAnchors.labels}
