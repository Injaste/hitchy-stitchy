...
import type { RoyalMuslimPageConfig } from "./types"
import {
  rsvpClassNames,
  rsvpLabels,
...
import { royalMuslimAnchors } from "./anchors"
import slugCss from "./styles.css?inline"

const Lottie = (LottieRaw as any).default ?? LottieRaw

// Regal Roman-caps names — a formal break from the script / Playfair templates.
const DEFAULT_FONTS = {
...
const CONFETTI_COLORS = ["#8f243d", "#b3923f", "#c9ab63", "#fdf6ec", "#6f1a2f"]

// ─── Ornament primitives ──────────────────────────────────────────────────────

const Medallion = ({ size = 48, className = "" }: { size?: number; className?: string }) => (
  <svg viewBox="0 0 100 100" width={size} height={size} className={className} aria-hidden fill="none" stroke="currentColor">
    <circle cx={50} cy={50} r={46} strokeWidth={1.4} />
    <circle cx={50} cy={50} r={37} strokeWidth={0.7} opacity={0.6} />
    ...
    <polygon points="50,34 58,50 50,66 42,50" fill="currentColor" stroke="none" />
    <circle cx={50} cy={50} r={3} fill="var(--ry-bg)" stroke="none" />
  </svg>
)

const OrnateDivider = ({ className = "" }: { className?: string }) => (
  <div className={cn("flex items-center justify-center gap-3 text-(--ry-gold)", className)}>
...
// Gilded double-border panel with corner studs.
const OrnateFrame = ({ className = "", children }: { className?: string; children: React.ReactNode }) => (
  <div className={cn("relative border border-(--ry-gold)/60 bg-(--ry-card)/60", className)}>
    <div className="pointer-events-none absolute inset-1.5 border border-(--ry-gold)/30" />
    <span className="absolute -top-1 -left-1 size-2 rotate-45 bg-(--ry-gold)" />
    <span className="absolute -top-1 -right-1 size-2 rotate-45 bg-(--ry-gold)" />
    <span className="absolute -bottom-1 -left-1 size-2 rotate-45 bg-(--ry-gold)" />
    <span className="absolute -bottom-1 -right-1 size-2 rotate-45 bg-(--ry-gold)" />
    ...
    <div className="flex items-center justify-center">
      {UNITS.map((u, i) => (
        <div key={u.key} className="flex items-center">
          {i > 0 && <span className="mx-3 size-1.5 rotate-45 bg-(--ry-gold)/60 shrink-0" />}
          <div className="flex flex-col items-center w-16">
            <span className="ry-countdown-number text-3xl text-(--ry-burgundy) tabular-nums leading-none">
              {String(t[u.key]).padStart(2, "0")}
            </span>
            <span className="mt-1.5 text-3xs uppercase tracking-[0.2em] text-(--ry-muted-fg)">{u.label}</span>
          </div>
        </div>
      ))}
    </div>
...
// ─── Medallion-bloom preloader ─────────────────────────────────────────────────

const HOLD_MS = 1000

const MedallionPreloader = ({
  loaderReady,
  onExitComplete,
}: {
  loaderReady: boolean
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div
          className="fixed inset-0 z-100 flex items-center justify-center bg-(--ry-bg)"
          initial={{ opacity: 1 }}
...
const RoyalMuslim = ({ eventConfig, pageConfig, loaderReady }: ThemeProps) => {
  const config = (pageConfig ?? {}) as Partial<RoyalMuslimPageConfig>
  ...
  const renderClosed = (key: string, message: string) => (
    <motion.p
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="text-center text-(--ry-muted-fg) italic leading-relaxed py-8"
    >
      {message}
    </motion.p>
  )
  ...
  const renderSuccess = () => (
    <div key="success" className="text-center">
      ...
      >
        <h3 className="text-2xl font-medium my-3 text-(--ry-burgundy) italic">{config.rsvp_success_heading}</h3>
        <motion.p
          initial={rsvp.submitted ? { opacity: 0 } : false}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.3, duration: 0.6, ease: "easeOut" }}
          className="text-(--ry-muted-fg) leading-relaxed italic mb-6"
        >
        ...
        >
          <Button
            ...
            className="rounded-sm bg-transparent text-(--ry-fg) border-(--ry-gold)/50 hover:bg-(--ry-bg-2) gap-2 font-semibold tracking-widest uppercase text-xs shrink-0"
          >
            <Edit2 size={14} className="text-(--ry-burgundy)" /> Edit
          </Button>
          <Button
            variant="outline"
            size="sm"
            ...
            className="rounded-sm bg-transparent text-(--ry-fg) border-(--ry-gold)/50 hover:border-(--ry-destructive)/50 hover:text-(--ry-destructive) gap-2 font-semibold tracking-widest uppercase text-xs shrink-0"
          >
            <Trash2 size={14} className="text-(--ry-burgundy)" />
  ...
  const anchorItems = royalMuslimAnchors.items.filter((item) => !item.when || item.when(config))

  return (
    <div className="ry-root relative min-h-svh bg-(--ry-bg) text-(--ry-fg)" style={rootStyle}>
      <MedallionPreloader loaderReady={!!loaderReady} onExitComplete={() => setReady(true)} />

      {/* ── Hero ── medallion crest, regal names, ornate flourishes */}
      <section
        id="hero"
      ...
      >
        <motion.div initial="hidden" animate={ready ? "show" : "hidden"} className="w-full max-w-lg mx-auto">
          <motion.div variants={heroMake(0)} className="text-(--ry-gold) flex justify-center mb-8">
            <Medallion size={56} />
          </motion.div>

          <motion.p
            variants={heroMake(0.2)}
            className="text-(--ry-muted-fg) text-base italic mb-8 whitespace-pre-line"
          >
            {config.greeting}
          </motion.p>

          <motion.div variants={heroMake(0.4)}>
            <OrnateDivider className="mb-6" />
          </motion.div>

          <motion.p
            variants={heroMake(0.55)}
            className="text-(--ry-gold) text-xs tracking-[0.4em] uppercase mb-6"
          >
          ...
          <motion.h1
            ...
            className="ry-couple-names text-4xl sm:text-5xl uppercase text-(--ry-burgundy) leading-tight"
          >
            {config.groom_name}
          </motion.h1>
          <motion.p variants={heroMake(1.0)} className="ry-couple-names text-2xl text-(--ry-gold) my-3">
            &amp;
          </motion.p>
          <motion.h1
          ...
          <motion.div variants={heroMake(1.8)} className="mt-10">
            <OrnateDivider className="mb-6" />
            <p className="text-(--ry-fg)/80 leading-relaxed italic whitespace-pre-line max-w-md mx-auto">
              {config.quote}
            </p>
            <span className="block mt-3 text-(--ry-muted-fg) text-xs tracking-[0.25em] uppercase">
              {config.quote_source}
            </span>
          </motion.div>
        </motion.div>
      ...
      {/* ── Invitation + Details ── gilded framed panel */}
      <section id="details" className="py-20 px-6 bg-(--ry-bg-2)/40">
        <motion.div
          initial="hidden"
          whileInView="show"
        ...
        >
          <OrnateFrame className="p-8 sm:p-10 text-center">
            <motion.div variants={fadeIn(0)} className="text-(--ry-gold) flex justify-center mb-5">
              <Medallion size={30} />
            </motion.div>
            <motion.h2 variants={fadeUp(0.1, 14, 0.7)} className="text-3xl font-medium italic text-(--ry-burgundy) mb-5">
              {config.section_title ?? "The Invitation"}
            </motion.h2>
            {config.invitation_body && (
              <motion.p variants={fadeUp(0.2, 14, 0.8)} className="text-(--ry-muted-fg) leading-relaxed">
                {config.invitation_body}
              </motion.p>
            )}
            ...
            {(config.blessings_name || config.blessings_label) && (
              <>
                <motion.div variants={fadeIn(0.25)}>
                  <OrnateDivider className="my-8" />
                </motion.div>
                <motion.p variants={fadeIn(0.3)} className="text-(--ry-muted-fg) text-sm mb-2">
                  {config.blessings_prefix}
                </motion.p>
                {config.blessings_name && (
                  <motion.h3 variants={fadeUp(0.35, 12, 0.7)} className="text-2xl font-medium text-(--ry-burgundy) whitespace-pre-line italic">
                    {config.blessings_name}
                  </motion.h3>
                )}
                {config.blessings_label && (
                  <motion.p variants={fadeUp(0.4, 10, 0.6)} className="text-(--ry-muted-fg) text-sm mt-1">
                    {config.blessings_label}
                  </motion.p>
                )}
              </>
              ...
              <motion.div variants={fadeIn(0.3)} className="mt-8 flex flex-col">
                {detailsList.map((item, idx) => (
                  <motion.div
                    ...
                    className="py-3.5 border-t border-(--ry-gold)/25 last:border-b"
                  >
                    <p className="text-2xs uppercase tracking-[0.3em] text-(--ry-gold) mb-1">{item.title}</p>
                    <p className="text-lg text-(--ry-fg)">{item.detail}</p>
                  </motion.div>
                ))}
              </motion.div>
            )}
      ...
      {/* ── Itinerary ── ornate flourish-separated programme */}
        ...
        <section id="itinerary" className="py-20 px-6">
          <div className="max-w-md mx-auto text-center">
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }} className="mb-10">
              <motion.div variants={fadeIn(0)} className="text-(--ry-gold) flex justify-center mb-4">
                <Medallion size={26} />
              </motion.div>
              <motion.h2 variants={fadeUp(0.1, 14, 0.7)} className="text-3xl font-medium italic text-(--ry-burgundy)">
            ...
            <div className="flex flex-col">
              {itinerarySections.map((section, si) => (
                ...
                >
                  {si > 0 && <OrnateDivider className="my-7" />}
                  <p className="ry-couple-names text-xl uppercase text-(--ry-burgundy) mb-3">{section.title}</p>
                  <div className="flex flex-col gap-1.5 text-(--ry-muted-fg)">
                    {section.items.map((item, ii) => (
                      <div key={ii} className="flex items-baseline justify-center gap-3 text-sm">
                        <span className="tabular-nums text-(--ry-gold)">{item.time}</span>
            ...
            {config.footnote && (
              <motion.p
                ...
                viewport={{ once: true, margin: "-40px" }}
                variants={fadeIn(0.1)}
                className="mt-10 text-(--ry-muted-fg) italic text-sm"
              >
                {config.footnote}
              </motion.p>
            )}
      ...
      {/* ── RSVP ── gilded framed panel */}
      <section ref={rsvp.sectionRef} id="rsvp" className="py-20 px-6 bg-(--ry-bg-2)/40">
        <div className="max-w-md mx-auto">
          ...
          >
            <OrnateFrame className="p-8">
              <div className="text-center mb-8">
                <motion.div variants={fadeIn(0)} className="text-(--ry-gold) flex justify-center mb-4">
                  <Medallion size={30} />
                </motion.div>
                <motion.h2 variants={fadeUp(0.1, 12, 0.6)} className="text-3xl font-medium text-(--ry-burgundy) italic">
                  RSVP
                </motion.h2>
                <motion.p variants={fadeUp(0.2, 10, 0.6)} className="text-(--ry-muted-fg) italic mt-2 text-sm">
          ...
          >
            ...
            <motion.p variants={fadeUp(0.1, 10, 0.6)} className="text-2xs uppercase tracking-[0.3em] text-(--ry-muted-fg) mb-3">
              {config.footer_tagline}
            </motion.p>
            <motion.h2
              variants={fadeUp(0.2, 12, 0.7)}
              className="ry-couple-names text-2xl uppercase text-(--ry-burgundy)"
            >
      ...
      <AnchorDock
        ...
        classNames={royalMuslimAnchors.classNames}
        drawerClassNames={royalMuslimAnchors.drawer}
        labels={royalMuslimAnchors.labels}
        calendar={{
