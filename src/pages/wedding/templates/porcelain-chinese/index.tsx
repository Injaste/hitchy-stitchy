...
import type { PorcelainChinesePageConfig } from "./types"
import { rsvpClassNames, rsvpLabels, rsvpDeleteClassNames, rsvpDeleteLabels } from "./form"
import { porcelainChineseAnchors } from "./anchors"
...
const CONFETTI_COLORS = ["#2f5d8c", "#3b7fb5", "#cfe0ee", "#ffffff", "#234a73"]

// ─── Porcelain medallion ──────────────────────────────────────────────────────

const Medallion = ({ size = 56, className = "" }: { size?: number; className?: string }) => (
  <svg viewBox="0 0 100 100" width={size} height={size} className={className} aria-hidden fill="none" stroke="var(--pc-primary)">
    <circle cx={50} cy={50} r={46} strokeWidth={1.4} />
    <circle cx={50} cy={50} r={37} strokeWidth={0.7} opacity={0.55} />
    {Array.from({ length: 8 }).map((_, i) => (
      <path key={i} d="M50 13 q5.5 8 0 16 q-5.5 -8 0 -16 Z" transform={`rotate(${i * 45} 50 50)`} strokeWidth={1} fill="var(--pc-primary)" fillOpacity={0.14} />
    ))}
    <circle cx={50} cy={50} r={7} fill="var(--pc-primary)" stroke="none" />
  </svg>
)

const ScrollDivider = ({ className = "" }: { className?: string }) => (
  <div className={cn("flex items-center justify-center gap-3 text-(--pc-primary)/55", className)}>
    ...
    <div className="flex items-end justify-center gap-6">
      {UNITS.map((u) => (
        <div key={u.key} className="flex flex-col items-center w-16">
          <span className="pc-countdown-number text-3xl text-(--pc-primary) tabular-nums leading-none">{String(t[u.key]).padStart(2, "0")}</span>
          <span className="mt-1.5 text-3xs uppercase tracking-[0.2em] text-(--pc-muted-fg)">{u.label}</span>
        </div>
      ))}
    </div>
  )
...
const Preloader = ({ loaderReady, onExitComplete }: { loaderReady: boolean; onExitComplete: () => void }) => {
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div className="fixed inset-0 z-100 flex items-center justify-center bg-(--pc-bg)" initial={{ opacity: 1 }} animate={{ opacity: leaving ? 0 : 1 }} transition={{ duration: 0.8, ease: "easeInOut" }} onAnimationComplete={() => { if (leaving) setVisible(false) }}>
          <motion.div initial={{ opacity: 0, scale: 0.5, rotate: -30 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}>
            <Medallion size={92} />
          </motion.div>
        </motion.div>
      )}
...
const PorcelainChinese = ({ eventConfig, pageConfig, loaderReady }: ThemeProps) => {
  const config = (pageConfig ?? {}) as Partial<PorcelainChinesePageConfig>
  const fonts = useMemo(() => ({
  ...
  const renderClosed = (key: string, message: string) => (
    <motion.p key={key} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center text-(--pc-muted-fg) italic leading-relaxed py-8">{message}</motion.p>
  )
  const renderSuccess = () => (
    <div key="success" className="text-center">
      ...
      <motion.div key="success-content" initial={rsvp.submitted ? { opacity: 0, y: 12 } : false} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
        <h3 className="text-2xl font-medium my-3 text-(--pc-primary) italic">{config.rsvp_success_heading}</h3>
        <motion.p initial={rsvp.submitted ? { opacity: 0 } : false} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.6, ease: "easeOut" }} className="text-(--pc-muted-fg) leading-relaxed italic mb-6">{eventConfig.confirmation_message}</motion.p>
        <motion.div initial={rsvp.submitted ? { opacity: 0 } : false} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.5, ease: "easeOut" }} className="flex gap-3 justify-center">
          <Button variant="outline" size="sm" onClick={() => rsvp.setIsEditing(true)} className="rounded-md bg-transparent text-(--pc-fg) border-(--pc-primary)/30 hover:bg-(--pc-bg-2) gap-2 font-semibold tracking-widest uppercase text-xs shrink-0"><Edit2 size={14} className="text-(--pc-primary)" /> Edit</Button>
          <Button variant="outline" size="sm" disabled={rsvp.removePending} onClick={() => rsvp.setShowDeleteDialog(true)} className="rounded-md bg-transparent text-(--pc-fg) border-(--pc-primary)/30 hover:border-(--pc-destructive)/50 hover:text-(--pc-destructive) gap-2 font-semibold tracking-widest uppercase text-xs shrink-0"><Trash2 size={14} className="text-(--pc-primary)" />{rsvp.removePending ? "Removing…" : "Delete"}</Button>
        </motion.div>
      </motion.div>
    </div>
  )
  ...
  const anchorItems = porcelainChineseAnchors.items.filter((item) => !item.when || item.when(config))

  return (
    <div className="pc-root relative min-h-svh bg-(--pc-bg) text-(--pc-fg)" style={rootStyle}>
      ...
      <section id="hero" className="relative min-h-svh flex flex-col items-center justify-center text-center px-6 py-20">
        <motion.div initial="hidden" animate={ready ? "show" : "hidden"} className="w-full max-w-lg mx-auto">
          <motion.div variants={heroMake(0)} className="flex justify-center mb-8"><Medallion size={64} /></motion.div>
          <motion.p variants={heroMake(0.2)} className="text-(--pc-muted-fg) text-base italic mb-8 whitespace-pre-line pc-hanzi">{config.greeting}</motion.p>
          <motion.div variants={heroMake(0.4)}><ScrollDivider className="mb-6" /></motion.div>
          <motion.p variants={heroMake(0.55)} className="text-(--pc-accent) text-xs tracking-[0.4em] uppercase mb-5">{config.hero_divider_label}</motion.p>
          <motion.h1 variants={heroMake(0.75, 22, 1)} className="pc-couple-names text-4xl sm:text-5xl uppercase text-(--pc-primary) leading-tight">{config.groom_name}</motion.h1>
          <motion.p variants={heroMake(1.0)} className="pc-couple-names text-2xl text-(--pc-accent) my-3">&amp;</motion.p>
          <motion.h1 variants={heroMake(1.15, 22, 1)} className="pc-couple-names text-4xl sm:text-5xl uppercase text-(--pc-primary) leading-tight">{config.bride_name}</motion.h1>
          ...
          <motion.div variants={heroMake(1.8)} className="mt-10">
            <ScrollDivider className="mb-6" />
            <p className="text-(--pc-fg)/80 leading-relaxed italic whitespace-pre-line max-w-md mx-auto pc-hanzi">{config.quote}</p>
            <span className="block mt-3 text-(--pc-muted-fg) text-xs tracking-[0.25em] uppercase">{config.quote_source}</span>
          </motion.div>
        </motion.div>
      </section>
      ...
      <section id="details" className="py-20 px-6">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} className="max-w-lg mx-auto rounded-lg border-2 border-(--pc-primary)/20 bg-(--pc-card)/85 p-8 sm:p-10 text-center shadow-sm">
          <motion.div variants={fadeIn(0)} className="flex justify-center mb-5"><Medallion size={40} /></motion.div>
          <motion.h2 variants={fadeUp(0.1, 14, 0.7)} className="text-3xl font-medium italic text-(--pc-primary) mb-5">{config.section_title ?? "A Celebration of Love"}</motion.h2>
          {config.invitation_body && <motion.p variants={fadeUp(0.2, 14, 0.8)} className="text-(--pc-muted-fg) leading-relaxed">{config.invitation_body}</motion.p>}
          {(config.blessings_name || config.blessings_label) && (
            <motion.div variants={fadeIn(0.25)} className="mt-8">
              <p className="text-(--pc-muted-fg) text-sm mb-2">{config.blessings_prefix}</p>
              {config.blessings_name && <h3 className="text-2xl font-medium text-(--pc-primary) whitespace-pre-line italic">{config.blessings_name}</h3>}
              {config.blessings_label && <p className="text-(--pc-muted-fg) text-sm mt-1">{config.blessings_label}</p>}
            </motion.div>
          )}
          {detailsList.length > 0 && (
            <motion.div variants={fadeIn(0.3)} className="mt-8 flex flex-col">
              {detailsList.map((item, idx) => (
                <motion.div key={idx} variants={fadeUp(idx * 0.08, 10, 0.6)} className="py-3.5 border-t border-(--pc-border) last:border-b">
                  <p className="text-2xs uppercase tracking-[0.3em] text-(--pc-accent) mb-1">{item.title}</p>
                  <p className="text-lg text-(--pc-fg)">{item.detail}</p>
                </motion.div>
              ))}
            </motion.div>
          )}
        ...
        <section id="itinerary" className="py-20 px-6">
          <div className="max-w-md mx-auto text-center">
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }} className="mb-10">
              <motion.div variants={fadeIn(0)} className="flex justify-center mb-4"><Medallion size={34} /></motion.div>
              <motion.h2 variants={fadeUp(0.1, 14, 0.7)} className="text-3xl font-medium italic text-(--pc-primary)">{config.itinerary_title ?? "Programme"}</motion.h2>
            </motion.div>
            <div className="flex flex-col">
              {itinerarySections.map((section, si) => (
                <motion.div key={si} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-40px" }} variants={fadeUp(0, 14, 0.6)}>
                  {si > 0 && <ScrollDivider className="my-7" />}
                  <p className="pc-couple-names text-xl uppercase text-(--pc-primary) mb-3">{section.title}</p>
                  <div className="flex flex-col gap-1.5 text-(--pc-muted-fg)">
                    {section.items.map((item, ii) => (
                      <div key={ii} className="flex items-baseline justify-center gap-3 text-sm">
                        <span className="tabular-nums text-(--pc-accent)">{item.time}</span>
            ...
            {config.footnote && <motion.p initial="hidden" whileInView="show" viewport={{ once: true, margin: "-40px" }} variants={fadeIn(0.1)} className="mt-10 text-(--pc-muted-fg) italic text-sm">{config.footnote}</motion.p>}
          </div>
        </section>
      )}
      ...
      <section ref={rsvp.sectionRef} id="rsvp" className="py-20 px-6">
        <div className="max-w-md mx-auto">
          <motion.div layout transition={{ layout: { duration: 0.45, ease: [0.16, 1, 0.3, 1] } }} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }} variants={fadeIn(0, 0.8)} className="rounded-lg border-2 border-(--pc-primary)/20 bg-(--pc-card) p-8 shadow-sm">
            <div className="text-center mb-8">
              <motion.div variants={fadeIn(0)} className="flex justify-center mb-4"><Medallion size={40} /></motion.div>
              <motion.h2 variants={fadeUp(0.1, 12, 0.6)} className="text-3xl font-medium text-(--pc-primary) italic">RSVP</motion.h2>
              <motion.p variants={fadeUp(0.2, 10, 0.6)} className="text-(--pc-muted-fg) italic mt-2 text-sm">{config.rsvp_subtitle}</motion.p>
            </div>
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-40px" }} className="mt-14 text-center">
            <motion.div variants={fadeIn(0)} className="flex justify-center mb-5"><Medallion size={30} /></motion.div>
            <motion.p variants={fadeUp(0.1, 10, 0.6)} className="text-2xs uppercase tracking-[0.3em] text-(--pc-muted-fg) mb-3">{config.footer_tagline}</motion.p>
            <motion.h2 variants={fadeUp(0.2, 12, 0.7)} className="pc-couple-names text-2xl uppercase text-(--pc-primary)">{config.groom_name} &amp; {config.bride_name}</motion.h2>
          </motion.div>
        </div>
      ...
      <AnchorDock ready={ready} eventConfig={eventConfig} scrollItems={anchorItems}
        classNames={porcelainChineseAnchors.classNames} drawerClassNames={porcelainChineseAnchors.drawer} labels={porcelainChineseAnchors.labels}
...
export default PorcelainChinese
