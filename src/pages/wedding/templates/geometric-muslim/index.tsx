...
const KhatamStar = ({ size = 40, className = "" }: { size?: number; className?: string }) => (
  <svg viewBox="0 0 100 100" width={size} height={size} className={className} aria-hidden>
    <polygon points={starPoints(50, 50, 47, 21)} fill="none" stroke="currentColor" strokeWidth={2} />
    <polygon points={starPoints(50, 50, 31, 14)} fill="none" stroke="currentColor" strokeWidth={1.5} opacity={0.7} />
    <circle cx={50} cy={50} r={5.5} fill="currentColor" />
  </svg>
)

// Double-lined pointed (mihrab) arch — the template's signature motif.
const MihrabArch = ({ className = "" }: { className?: string }) => (
  ...
  >
    <path
      d="M22 150 L22 92 A 78 78 0 0 1 100 16 A 78 78 0 0 1 178 92 L178 150"
      stroke="currentColor"
      strokeWidth={1.5}
    />
    <path
      d="M36 150 L36 98 A 64 64 0 0 1 100 34 A 64 64 0 0 1 164 98 L164 150"
      stroke="currentColor"
      strokeWidth={0.75}
      opacity={0.5}
    />
...
const heroArch: Variants = {
  hidden: { opacity: 0, y: -20 },
  show: { opacity: 1, y: 0, transition: { duration: 1, delay: HERO_BASE, ease: [0.16, 1, 0.3, 1] } },
}
...
const InlineCountdown = ({ target }: { target: Date | null }) => {
  ...
  return (
    <div className="flex items-stretch justify-center divide-x divide-(--gm-primary)/20">
      {UNITS.map((u) => (
        <div key={u.key} className="flex flex-col items-center px-5">
...
const SectionHeading = ({ index, title }: { index: number; title: string }) => (
  <div className="mb-12">
    <motion.div variants={fadeIn(0)} className="flex items-center gap-3 text-(--gm-primary)">
      <span className="gm-countdown-number text-lg text-(--gm-primary)/60">{ROMAN[index]}</span>
      <motion.span variants={lineGrow} style={{ originX: 0 }} className="h-px w-12 bg-(--gm-primary)/40" />
    </motion.div>
    <motion.h2
...
const StarPreloader = ({
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div
          className="fixed inset-0 z-100 flex items-center justify-center bg-(--gm-bg)"
          initial={{ opacity: 1 }}
...
const GeometricBackground = () => (
  <div className="fixed inset-0 pointer-events-none" aria-hidden>
    ...
    <svg className="absolute inset-0 w-full h-full opacity-[0.05]">
      <defs>
        <pattern id="gm-lattice" width={72} height={72} patternUnits="userSpaceOnUse">
          <polygon points={starPoints(36, 36, 13, 5.5)} fill="none" stroke="var(--gm-primary)" strokeWidth={1} />
          <line x1={36} y1={0} x2={36} y2={9} stroke="var(--gm-primary)" strokeWidth={1} />
          <line x1={36} y1={63} x2={36} y2={72} stroke="var(--gm-primary)" strokeWidth={1} />
          <line x1={0} y1={36} x2={9} y2={36} stroke="var(--gm-primary)" strokeWidth={1} />
          <line x1={63} y1={36} x2={72} y2={36} stroke="var(--gm-primary)" strokeWidth={1} />
        </pattern>
      </defs>
...
const GeometricMuslim = ({ eventConfig, pageConfig, loaderReady }: ThemeProps) => {
  ...
  let sectionNo = -1
  const detailsNo = (detailsList.length || config.invitation_body) ? ++sectionNo : sectionNo
  ...
  const renderSuccess = () => (
    <div key="success" className="text-center">
      <Lottie animationData={successCheck} loop={false} style={{ width: 80, height: 80, margin: "0 auto" }} />
      <motion.div
        key="success-content"
      ...
      >
        <h3 className="text-2xl font-bold my-3 text-(--gm-gold-soft)">{config.rsvp_success_heading}</h3>
        <motion.p
  ...
  const anchorItems = geometricMuslimAnchors.items.filter((item) => !item.when || item.when(config))

  return (
    <div className="gm-root relative min-h-svh text-(--gm-fg)" style={rootStyle}>
      <StarPreloader loaderReady={!!loaderReady} onExitComplete={() => setReady(true)} />
      <GeometricBackground />

      {/* ── Hero ── arch crown + star-separated names + inline countdown */}
      <section
        id="hero"
        className="relative min-h-svh flex flex-col items-center justify-center text-center pb-16 pt-16 px-6 overflow-hidden"
      >
        <motion.div initial="hidden" animate={ready ? "show" : "hidden"} className="z-10 w-full max-w-xl mx-auto">
          <motion.div variants={heroArch} className="relative text-(--gm-primary) mx-auto w-52 mb-8">
            <MihrabArch className="w-full h-28" />
            <div className="absolute inset-x-0 top-3 flex justify-center text-(--gm-gold-soft)">
              <KhatamStar size={34} />
            </div>
          </motion.div>
          ...
          <div className="mb-12">
            ...
            <motion.div variants={heroMake(1.3)} className="text-(--gm-primary) flex justify-center my-3">
              <KhatamStar size={26} />
            </motion.div>
            <motion.span
          ...
          <motion.div variants={heroMake(2.5)}>
            <p className="text-(--gm-fg) leading-relaxed whitespace-pre-line italic">{config.quote}</p>
      ...
      <section id="details" className="py-20 px-6 relative z-10">
        <div className="max-w-lg mx-auto">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }}>
            <SectionHeading index={detailsNo} title={config.section_title ?? "The Invitation"} />
            {config.invitation_body && (
              <motion.p variants={fadeUp(0.2, 16, 0.8)} className="text-(--gm-fg)/70 leading-relaxed mb-12">
                {config.invitation_body}
              </motion.p>
            )}
          </motion.div>
          ...
          {(config.blessings_name || config.blessings_label) && (
            ...
            >
              <motion.p variants={fadeIn(0)} className="text-(--gm-muted-fg) text-sm mb-2">
              ...
              {config.blessings_label && (
                <motion.p variants={fadeUp(0.2, 10, 0.6)} className="text-(--gm-fg)/60 text-sm mt-1">
                  {config.blessings_label}
                </motion.p>
              )}
            </motion.div>
              ...
              {detailsList.map((item, idx) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 flex items-baseline justify-between gap-4">
                    ...
                    <span className="text-(--gm-gold-soft) font-semibold text-right">{item.detail}</span>
                  </div>
                </motion.div>
              ))}
            </motion.div>
        ...
        <section id="itinerary" className="py-20 px-6 relative z-10">
          <div className="max-w-lg mx-auto">
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }}>
              <SectionHeading index={itineraryNo} title={config.itinerary_title ?? "Programme"} />
            </motion.div>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1.5">
                      {section.items.map((item, ii) => (
                        <div key={ii} className="flex items-baseline gap-4 text-sm">
                          <span className="tabular-nums shrink-0 text-(--gm-primary) w-20">{item.time}</span>
                          {item.label && <span className="text-(--gm-fg)/80">{item.label}</span>}
                        </div>
                      ))}
                    </div>
                  </motion.div>
        ...
        <div className="max-w-md mx-auto">
          ...
          >
            <div className="text-center mb-10">
              <motion.div variants={fadeIn(0)} className="relative text-(--gm-primary) mx-auto w-32 mb-2">
                <MihrabArch className="w-full h-16" />
                <div className="absolute inset-x-0 top-1 flex justify-center text-(--gm-gold-soft)">
                  <KhatamStar size={20} />
                </div>
              </motion.div>
              <motion.h2
                variants={fadeUp(0.1, 14, 0.6)}
                className="text-3xl font-bold text-(--gm-gold-soft) italic"
              >
                RSVP
              </motion.h2>
              <motion.p variants={fadeUp(0.2, 10, 0.6)} className="text-(--gm-muted-fg) italic mt-2 text-sm">
          ...
          {/* ── Footer ── arch ornament with names */}
          <motion.div
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-40px" }}
          ...
          >
            <motion.div variants={fadeIn(0)} className="relative text-(--gm-primary)/70 mx-auto w-40">
              <MihrabArch className="w-full h-20" />
              <div className="absolute inset-0 flex flex-col items-center justify-end pb-1">
                <motion.p variants={fadeUp(0.1, 10, 0.6)} className="text-2xs uppercase tracking-[0.3em] text-(--gm-muted-fg) mb-1">
                  {config.footer_tagline}
                </motion.p>
                <p className="gm-couple-names text-2xl text-(--gm-gold-soft) leading-none">
