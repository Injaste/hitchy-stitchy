...
function deriveDays(range: { from: Date; to: Date }, prevDays: EventDay[]): EventDay[] {
...
export function EventSettingsSection() {
  ...
  const [rsvpDeadlineEnabled, setRsvpDeadlineEnabled] = useState(eventConfig.rsvpDeadlineEnabled);
        ...
        <CardContent className="space-y-6">
          ...
          {days.length > 0 && (
            <div className="space-y-4">
              {days.map((day, index) => (
                <div key={day.id} className="space-y-3 p-4 rounded-lg border border-border bg-muted/20">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-48 justify-start gap-2 font-normal">
                      <CalendarIcon className="h-4 w-4 text-muted-foreground" />
                      <span>
                        {rsvpDeadline ? format(rsvpDeadline, "do MMM yyyy") : "Pick a date"}
                      </span>
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              value={rsvpMode}
              onValueChange={(v) => setRsvpMode(v as EventConfig["rsvpForm"]["mode"])}
              className="space-y-2"
            >
              {[
