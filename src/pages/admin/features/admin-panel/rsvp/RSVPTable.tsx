          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left border-collapse">
              ...
              <tbody className="divide-y divide-border">
                {rsvps.map((rsvp) => (
                  <tr key={rsvp.id} className="hover:bg-muted/40 transition-colors">
                    <td className="px-4 py-3 font-medium text-foreground">
                      {rsvp.name}
                      <p className="text-[10px] text-muted-foreground font-normal">{rsvp.email}</p>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">{rsvp.guests + 1}</td>
                    <td className="px-4 py-3">
                      <Badge variant={statusVariant[rsvp.status]} className="text-[10px]">
                        {rsvp.status}
                      </Badge>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <Select
                        value={rsvp.status}
                        onValueChange={(val) =>
                          updateStatus.mutate({ id: rsvp.id, status: val as RSVP["status"] })
                        }
                      >
                        <SelectTrigger size="sm" className="text-[10px] font-bold w-28">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
