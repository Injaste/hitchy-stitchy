...
export function AssigneeCheckboxes({ teamRoles, defaultAssignees, feature }: Props) {
  return (
    <div className="bg-card border border-border rounded-md p-3 max-h-40 overflow-y-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor={`${feature}-assignee-All`} className="text-sm cursor-pointer">All</label>
        </div>
        {teamRoles.map((r) => (
          <div key={r.role} className="flex items-center gap-2">
            ...
            />
            <label htmlFor={`${feature}-assignee-${r.role}`} className="text-sm cursor-pointer">
